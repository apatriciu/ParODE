...
__kernel void RootCopyKernelScanDownsweep(	__global fType* VectVSource,
	...
	int SourceIndex = ((get_global_id(0) / get_local_size(0)) + 1) * 2 * nElementStride - 1; // this is also the destination
	int DestinationIndex = SourceIndex - nElementStride ;
	int GlobalSourceVectBaseIndex = SourceIndex * nElements;
	...
	for(int StartLocalIndex = 0; StartLocalIndex < nElements; StartLocalIndex += get_local_size(0)){
		...
		if(LocalIndex < nElements)
			VectVDest[GlobalDestinationVectBaseIndex + LocalIndex] = VectVSource[GlobalSourceVectBaseIndex + LocalIndex];
	}
}
