...
__kernel void MatrixMatrixMultiplicationKernelScanDownsweep(__global fType* VectMSource, // vector with source matrices
	...
	// this kernel should compute the product between the matrix with low index and the matrix with high index (in this order)
	// the result should go into the high index
	int SourceBIndex = ((get_global_id(0) / get_local_size(0)) + 1) * 2 * nElementStride - 1; // this is also the destination
	int SourceAIndex = SourceBIndex - nElementStride ;
	...
	for(int rowTileStart = 0; rowTileStart < MatrixSize; rowTileStart += TileSize){
		...
		for(int columnTileStart = 0; columnTileStart < MatrixSize; columnTileStart += TileSize){
			...
			if(ResRow < MatrixSize && ResColumn < MatrixSize)
				VectMDest[SourceBIndex * MatrixSize * MatrixSize + ResRow * MatrixSize + ResColumn] = ResValue;
		}
	}
}
