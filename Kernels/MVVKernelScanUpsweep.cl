/*
Kernel for computing the matrix x vector + vector in the upsweep stage of the scan
...
__kernel void MatrixTimesVectorPlusVectorKernelScanUpsweep(	__global fType* VectMSource, 
	...
	for(int rowStart = 0; rowStart < MatrixSize; rowStart += get_local_size(1)){
		...
		if( (get_local_id(0) == 0) && (rowStart + get_local_id(1) < MatrixSize))
			VectVDest[SourceBIndex * MatrixSize + rowStart + get_local_id(1)] = localVect[get_local_id(1) * get_local_size(0)] + 
